...
class PeopleQueryTest < ActiveSupport::TestCase
  ...
  def test_test_object_scope_with_system_filter
    # Only system
    hash = {
      :f =>['is_system'],
      :op => {'is_system' => "="},
      :v => {'is_system' => [ActiveRecord::Base.connection.quoted_true.gsub(/'/, '')]}
    }
    ...
    hash = {
      :f =>['is_system'],
      :op => {'is_system' => "="},
      :v => {'is_system' => [ActiveRecord::Base.connection.quoted_false.gsub(/'/, '')]}
    }
